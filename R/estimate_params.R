...
estimate_params <- function(dataset, metadata = NULL, cellTypes = NULL, sex = NULL) {
  ...
  if (l == 0) { # Metadata-free case. Get rowMeans and UMIs per droplet and return.
    genedf <- rowMeans(dataset) # mean df
    names(genedf) <- rownames(dataset)
    sddf <- rowSds(dataset) # standard deviation df
    umis <- colSums(dataset)
    return(list(geneMeans = matrix(genedf, nrow = length(genedf)), geneSDs = matrix(sddf, nrow = length(sddf)), umis = matrix(umis, nrow = length(umis))))
  } else {
    ...
    genedf <- matrix(NA, ncol = nrow(tbl), nrow = nrow(dataset))
    sddf <- matrix(NA, ncol = nrow(tbl), nrow = nrow(dataset))
    ...
    for (i in 1:nrow(tbl)) {
      ...
      if (length(metadata_columns) > 1) {
        genedf[1:nrow(dataset),i] <- rowMeans(dataset[,metadata_columns])
        sddf[1:nrow(dataset),i] <- rowSds(dataset[,metadata_columns])
        umiVector <- colSums(dataset[,metadata_columns])
      } else {
        genedf[1:nrow(dataset),i] <- dataset[,metadata_columns]
        sddf[1:nrow(dataset),i] <- Inf
    ...
    rownames(genedf) <- rownames(dataset)
    rownames(sddf) <- rownames(dataset)
    rownames(umis) <- as.character(1:nrow(umis))

    return(list(geneMeans = genedf, geneSDs = sddf, umis = umis, metadata = tbl[,1:l]))
  }
}
